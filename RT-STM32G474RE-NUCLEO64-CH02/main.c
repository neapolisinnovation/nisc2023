  ...
  while( true ) {
    ...
    if( chX > 2.0f ) {
    	if (devices[0].status == 0){
    		devices[0].status = 1;
    	    palSetLine( devices[0].line );
    	}
    	if (devices[0].status == 1){
    	    		devices[0].status = 0;
    	    	    palClearLine( devices[0].line );
    	}
    }
    ...
    if( chY > 2.0f ) {
    	if (devices[1].status == 0){
    	    		devices[1].status = 1;
    	    	    palSetLine( devices[1].line );
    	}
    	if (devices[1].status == 1){
       	    		devices[1].status = 0;
      	    	    palClearLine( devices[1].line );
   	    }
    }
  }
...
int main(void) {
  ...
  //Device_t* devices = malloc(2 * sizeof(Device_t*));
  Device_t devices[2];

  devices[0].line = PAL_LINE(GPIOC, 7U);
  devices[0].status = 0;
  devices[1].line = PAL_LINE(GPIOA, 4U);
  devices[1].status = 0;
  //devices[2].line = PAL_LINE();
  //devices[2].status = 0;
  //devices[3].line = PAL_LINE();
  //devices[3].status = 0;

  chThdCreateStatic( waLed, sizeof( waLed), NORMALPRIO + 5, thdLed, (void*) devices );
